...
so a somersault and a stride come out the same size. All frames land horizontally centered
on ONE shared canvas.

Vertical alignment is per creature:
  'bottom' (default) -- every frame's lowest pixel sits on the floor (right for walkers).
  'ground'           -- frames are pinned to a SHARED ground line (the lowest-reaching
                        frame), so airborne frames float up -- a real hop/bounce for a
                        kangaroo, where a tight bottom-crop would flatten the arc.
...
set becomes a trick the daemon weaves into a crossing. Sources face RIGHT (daemon flips
for left travel).
...
def cells(path, rows, cols):
    ...
    bottom_in_cell) -- the cell row where the content ends, for ground alignment."""
...
def build(name, cfg):
    ...
    # per-frame vertical lift (source px): 'ground' floats airborne frames off a shared
    # ground line; 'bottom' keeps every frame's lowest pixel on the floor.
    lifts = {}
    ...
    uni_h = max(int(round(c.shape[0] * scales[s])) + int(round(lifts[s][i] * scales[s]))
                for s, fr in sets.items() for i, (c, _) in enumerate(fr))
    uni_w = max(int(round(c.shape[1] * scales[s])) for s, fr in sets.items() for c, _ in fr)
    ...
    for sname, fr in sets.items():
        ...
        for i, (c, _) in enumerate(fr):
            sw = max(1, round(c.shape[1] * s))
            sh = max(1, round(c.shape[0] * s))
            lift = int(round(lifts[sname][i] * s))
            img = Image.fromarray(c, 'RGBA').resize((sw, sh), Image.LANCZOS)
            canvas = Image.new('RGBA', (uni_w, uni_h), (0, 0, 0, 0))
            canvas.alpha_composite(img, ((uni_w - sw) // 2, uni_h - sh - lift))
